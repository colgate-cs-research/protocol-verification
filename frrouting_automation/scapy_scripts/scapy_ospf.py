...
class OSPF_Hdr(Packet):
    ...
    def post_build(self, p, pay):
        ...
        # See <http://tools.ietf.org/html/draft-ietf-ospf-lls-05>
        p += pay
        l = self.len
        if l is None:
            l = len(p)
...
class RepeatedTlvListField(PacketListField):
    # Code borrowed from scapy-cdp.py (Nicolas Bareil, Arnaud Ebalard)
...
class LLS_Extended_Options(Packet):
    name = "LLS Extended Options"
    fields_desc = [ ShortField("type", 1),
                    ShortField("len", 4),
                    LLS_ExtendedOptionsField()
                ]

class LLS_Crypto_Auth(Packet):
    name = "LLS Cryptographic Authentication"
    fields_desc = [ ShortField("type", 2),
                    FieldLenField("length", 20, fmt="B", length_of=lambda x: x.authdata),
                    XIntField("sequence", "\x00\x00\x00\x00"),
                    StrLenField("authdata", "\x00" * 16, length_from=lambda x: x.length),
                ]
...
def _LLSGuessPayloadClass(p, **kargs):
    ...
    if len(p) >= 4:
        ...
        clsname = _OSPF_LLSclasses.get(typ, "Raw")
        cls = globals()[clsname]
    return cls(p, **kargs)
...
class OSPF_LLS_Hdr(Packet):
    ...
    fields_desc = [ XShortField("chksum", None),
                    ShortField("len", None),
                    RepeatedTlvListField("llstlv", [], _LLSGuessPayloadClass)
                ]
