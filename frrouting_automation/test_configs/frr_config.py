...
def launch_topology(topology, routers, links, client, settings):
    '''Launch containers and bridges'''
    create_containers(routers, client, topology)
    create_networks(links, client)
    config_routers(routers, client, settings.bgpd)
...
def config_routers(routers, client, bgpd):
    for router_name in sorted(routers.keys()):
        router = routers[router_name]
...
def config_daemons(router):
    '''Configure daemons'''
    # Load template
    with open('daemons_temp', 'r') as file:
        template = file.read()
...
def config_vtysh(router):
    '''Configure vtysh'''
    # Load template
    with open('vtysh.conf_temp', 'r') as file:
        template = file.read()
...
def config_bgp(router, bgpd):
    print("Configuring BGP for "+router.name)

    # Load configuration template
    with open(bgpd, 'r') as file:
        template = file.read()
...
# Load configuration template
    with open('ospfd.conf_temp', 'r') as file:
        template = file.read()
...
def main():
    ...
    parser.add_argument("-b", "--bgpd", help="Path to bgpd config file", required=False)
    ...
    if (settings.action in ['start', 'restart']):
        if client.containers.list(filters={"label": topology}):
            ...
            launch_topology(topology, routers, links, client, settings)
